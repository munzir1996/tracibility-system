        ...
        <form action="{{route('cteagents.store')}}" method="POST">
            ...
            <div class="flex flex-col w-full rounded-lg shadow bg-white px-4 py-5">
                ...
                <div class="text-gray-600 font-bold text-right">
                    ...
                    <div class="mb-3">
                        <p class="inline-flex">{!!$qrcode->cteagent->cteHarvest->what->quantity!!} طن</p>
...
<div class="flex flex-row w-full">
    ...
    <div class="w-1/5  flex justify-center">
        <div class="relative flex h-full w-1 bg-green-300 items-center justify-center">
            <div class="absolute flex flex-col justify-center h-24 w-24 rounded-full border-2 border-green-300 leading-none text-center z-10 bg-white font-thin">
                <div>{{$qrcode->cteAgent->cteHarvest->import->when}}</div>
            </div>
        </div>
    </div>
    <!--right column-->
    <div class="w-2/5 px-2 py-10 ">
        <div class="flex flex-col w-full rounded-lg shadow bg-white px-4 py-5">
            ...
            <div class="text-gray-600 font-bold text-right">
                <div class="mb-3">
                    <p class="inline-flex">{{$qrcode->cteAgent->cteHarvest->import->user->name}}</p>
                    <label class="mb-0" for="">: المستلم</label>
                </div>
                <div class="mb-3">
                    <p class="inline-flex">{{$qrcode->cteAgent->cteHarvest->import->organization->name}}</p>
                    <label class="mb-0" for="">: الجهة</label>
                </div>
                <div class="mb-3">
                    <p class="inline-flex">{!!$qrcode->cteAgent->cteHarvest->what->quantity!!} طن</p>
                ...
                <div class="mb-3">
                    <span class="relative inline-block px-3 py-1 font-semibold  leading-tight
                    {{$qrcode->cteAgent->cteHarvest->import->why == Config::get('constants.delivery.received') ? 'text-green-900':''}}
                    {{$qrcode->cteAgent->cteHarvest->import->why == Config::get('constants.delivery.rejected') ? 'text-red-900':''}}">
                        <span aria-hidden="" class="absolute inset-0 opacity-50 rounded-full
                        {{$qrcode->cteAgent->cteHarvest->import->why == Config::get('constants.delivery.received') ? 'bg-green-200':''}}
                        {{$qrcode->cteAgent->cteHarvest->import->why == Config::get('constants.delivery.rejected') ? 'bg-red-200':''}}"></span>
                        <span class="relative">{{$qrcode->cteAgent->cteHarvest->import->why}}</span>
                    </span>
                    <label class="mb-0" for="">: الحالة</label>
                </div>
            </div>
        ...
        <form action="{{route('cteagents.store')}}" method="POST">
            ...
            <div class="flex flex-col w-full rounded-lg shadow bg-white px-4 py-5">
                ...
                <div class="text-gray-600 font-bold text-right">
                    ...
                    <div class="mb-3">
                        <p class="inline-flex">{!!$qrcode->cteAgent->what->gtin!!}</p>
                        <label class="mb-0" for="">: رقم البند</label>
                    </div>
                    <div class="mb-3">
                        <p class="inline-flex">{!!$qrcode->cteAgent->what->batch!!}</p>
                        <label class="mb-0" for="">: رقم الدفعة</label>
                    </div>
                    <div class="mb-3">
        ...
        <div class="flex flex-col w-full rounded-lg shadow bg-white px-4 py-5">
            ...
            <div class="text-gray-600 font-bold text-right">
                <div>
                    ...
                    <input type="hidden" name="cte_agent_id" value="{{$qrcode->cteAgent->id}}">
                </div>
